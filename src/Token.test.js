...
test.cb('revoked :: Token', t => {
  ...
  token
    ...
    .tapL(() =>
      t.fail(
        'must return Right(Token) when token not revoked, but it returned Left(Error)',
      ),
    )
  ...
  token
    ...
    .tap(() =>
      t.fail(
        'must return Left(Error) when token revoked, but it returned Right(Token)',
      ),
    )
})

test.cb('usable :: Token', t => {
  const token = of()
